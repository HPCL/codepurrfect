...
def create_tool_dirs(func_only=False, ast=False, pp=False): 
    '''
...
class PassRunner: 
    def __init__(self, initL : List[str], cg_pass : bool = True
        ...
        func_pass  -- Whether to run the *function-only* pass (default = False) 
        ...
        if cg_pass: 
            proj_root_dir, callfile, cgmetrics_file, \
                        qmfile, nodes_file, \
                        ll_res_path, call_res_path, \
                        ast_res_path, pp_res_path, qmetrics_path = create_tool_dirs(ast=ast_passes!=[], pp=pp_passes!=[])
    ...
    def run(self, pool : Pool, ast_passes : List[str] = [], pp_passes : List[str] = []) -> None: 
        '''
