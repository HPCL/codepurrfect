            ...
            for (CXXBaseSpecifier base_spec : cxx_record_decl -> bases())
            {
              ...
              if (!base_qty.isNull())
              {
                ...
                if (base_ty -> isStructureOrClassType())
                {
                  ...
                  if (base_cls)
                  {
                    // check whether base_cls has virtual destructor
                    CXXDestructorDecl* base_destr = base_cls -> getDestructor(); 
                    if (base_destr)
                    {
                      if (base_destr -> isVirtualAsWritten())
                      {
                        // check whether cxx_record_decl has virtual destructor.
                        CXXDestructorDecl* destr = cxx_record_decl -> getDestructor(); 
                        if (destr)
                        {
                          if (!(destr -> isVirtualAsWritten()))
                          {
                            report_file << "FOUND CHILD-DESTRUCTOR NOT VIRTUAL, "; 
                            SourceLocation b = destr -> getBeginLoc();
                        ...
                        {
                          report_file << "FOUND CHILD CLASS WITHOUT DESTRUCTOR, ";
                          SourceLocation b = cxx_record_decl -> getBeginLoc();
                          report_file << b.printToString(*(this -> src_manager)); 
                          report_file << "\n";
                        }
                      }
                      ...
                      {
                        report_file << "BASE DESTRUCTOR NOT VIRTUAL, ";
                        SourceLocation b = base_destr -> getBeginLoc();
                        report_file << b.printToString(*(this -> src_manager)); 
                        report_file << "\n";
                      }
...
public:
  virtual std::unique_ptr<clang::ASTConsumer> CreateASTConsumer(
    ...
    std::string metrics_filename = inFile_str.substr(0, dot_index) + "_cwe1045_metrics.csv";
